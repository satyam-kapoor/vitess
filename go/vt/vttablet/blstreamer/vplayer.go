...
func newVPlayer(bs *BlStreamer, settings binlogplayer.VRSettings, copyState map[string]*sqltypes.Result, pausePos mysql.Position) *vplayer {
	saveStop := true
	if !pausePos.IsZero() {
...
func (vp *vplayer) play(ctx context.Context) error {
	if !vp.stopPos.IsZero() && vp.startPos.AtLeast(vp.stopPos) {
		...
		if vp.saveStop {
			return vp.bs.stateStore.SetState(binlogplayer.BlpStopped, fmt.Sprintf("Stop position %v already reached: %v", vp.startPos, vp.stopPos))
		}
		return nil
	}

	plan, err := buildReplicatorPlan(vp.bs.source.Filter, vp.bs.tableKeys, vp.copyState)
	if err != nil {
		return err
	}
	...
	for _, rule := range vp.bs.source.Filter.Rules {
...
func (vp *vplayer) fetchAndApply(ctx context.Context) (err error) {
	log.Infof("Starting VReplication player id: %v, startPos: %v, stop: %v, filter: %v", vp.bs.vrId, vp.startPos, vp.stopPos, vp.bs.source.Filter)
	...
	go func() {
		streamErr <- vp.bs.vsClient.VStream(ctx, mysql.EncodePosition(vp.startPos), vp.replicatorPlan.VStreamFilter, func(events []*binlogdatapb.VEvent) error {
...
func (vp *vplayer) applyStmtEvent(ctx context.Context, event *binlogdatapb.VEvent) error {
	if vp.canAcceptStmtEvents {
		return vp.bs.sink.PutDml(ctx, event)
		
	}
	return fmt.Errorf("filter rules are not supported for SBR replication: %v", vp.bs.source.Filter.GetRules())
}

func (vp *vplayer) applyRowEvent(ctx context.Context, rowEvent *binlogdatapb.RowEvent) error {
	return vp.bs.sink.PutRow(ctx, rowEvent)
}

func (vp *vplayer) updatePos(ts int64) (posReached bool, err error) {
	gtid := mysql.EncodePosition(vp.pos)
	_ = ts //FIXME
	if err := vp.bs.stateStore.SaveLastPos(gtid); err != nil {
		return false, err
	}
	...
	if posReached {
		...
		if vp.saveStop {
			if err := vp.bs.stateStore.SetState(binlogplayer.BlpStopped, fmt.Sprintf("Stopped at position %v", vp.stopPos)); err != nil {
				return false, err
			}
		}
	}
...
func (vp *vplayer) applyEvent(ctx context.Context, event *binlogdatapb.VEvent, mustSave bool) error {
	...
	case binlogdatapb.VEventType_COMMIT:
		if mustSave {
			if err := vp.bs.dbClient.Begin(); err != nil {
				return err
			}
		}

		if !vp.bs.sink.IsInTransaction() {
		...
		if err := vp.bs.dbClient.Commit(); err != nil {
			return err
		}
		if posReached {
			return io.EOF
	...
	case binlogdatapb.VEventType_DDL:
		...
		case binlogdatapb.OnDDLAction_STOP:
			...
			if err := vp.bs.stateStore.SetState(binlogplayer.BlpStopped, fmt.Sprintf("Stopped at DDL %s", event.Ddl)); err != nil {
				return err
			}
			if err := vp.bs.dbClient.Commit(); err != nil {
				return err
		...
		case binlogdatapb.OnDDLAction_EXEC:
			...
			// failure here.
			if _, err := vp.bs.dbClient.ExecuteWithRetry(ctx, event.Ddl); err != nil {
				return err
			}
			posReached, err := vp.updatePos(event.Timestamp)
			if err != nil {
	...
	case binlogdatapb.VEventType_JOURNAL:
		...
		case binlogdatapb.MigrationType_TABLES:
			...
			case found && notFound:
				...
				if err := vp.bs.stateStore.SetState(binlogplayer.BlpStopped, "unable to handle journal event: tables were partially matched"); err != nil {
					return err
				}
				return io.EOF
			case notFound:
		...
		/*
		//FIXME: 
		if err := vp.bs.vre.registerJournal(event.Journal, int(vp.bs.vrId)); err != nil {
			if err := vp.bs.stateStore.SetState(binlogplayer.BlpStopped, err.Error()); err != nil {
				return err
			}
			return io.EOF
		}
